...
| GitHub Copilot | `AGENTS.md` + `.github/copilot-instructions.md`（可选） | `.github/` |
| OpenAI Codex | `AGENTS.md` | 项目根目录 |
| Windsurf | `AGENTS.md` + `.windsurfrules`（可选） | 项目根目录 |
| Cline / Roo Code | `AGENTS.md` + `.clinerules`（可选） | 项目根目录 |
| Kimi Code | `AGENTS.md` | 项目根目录或 `.kimi-code/` |
| Trae | `AGENTS.md` + `.trae/rules/*.md`（可选） | `.trae/rules/` |
| Gemini CLI / Firebase Studio | `AGENTS.md` + `GEMINI.md`（可选） | 项目根目录或 `.gemini/` |
| Antigravity | `AGENTS.md` + `GEMINI.md`（可选） | `.gemini/GEMINI.md` 或规则配置 |
...
1. 在 `guidelines/<section>/` 下新建 `.md` 文件（例如 `guidelines/working-standard/my-rule.md`）。
2. 添加 YAML frontmatter：`name`、`profiles`、`priority`，可选 `scopes`、`triggers`、`conflicts`。
3. 写一句加粗的核心句、展开 bullets、以及验收句（`**准则生效的标志：** ...`）。
4. 如果维护中文 atom，同时创建 `guidelines/<section>/my-rule.zh.md`，结构保持一致。
5. 重新生成 atom index：
   ```bash
...
6. 跑测试确保 index 一致：
   ...
   ```
7. 重新安装或 symlink skill 到你的 agent。

### 删除 atom

1. 从 `guidelines/<section>/` 删除 atom 文件（如有中文版本，同时删除 `.md` 和 `.zh.md`）。
2. 重新生成 index：
   ```bash
   python3 tools/build-index.py
   ```
